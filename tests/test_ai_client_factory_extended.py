...
from services.ai_client_factory import AIClientFactory, CachedClaudeClient, CachedGeminiClient
    ...
    def test_cached_claude_client_valid_cache_different_messages(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("anthropic.Anthropic") as mock_anthropic:
                ...
                # First call
                resp1 = client.create(model="claude-3", messages=[{"role": "user", "content": "Hello"}])
                assert resp1.content[0].text == "Response 1"

                # Second call with different message
                resp2 = client.create(model="claude-3", messages=[{"role": "user", "content": "Goodbye"}])
...
class TestPRServiceExtended:
            ...
            with patch("pathlib.Path.mkdir"):
                with patch("builtins.open", mock_open()) as mock_file:
                    with patch("subprocess.run") as mock_run:
                        ...
                        git_add_calls = [
                            call for call in mock_run.call_args_list if "git" in str(call) and "add" in str(call)
                        ]
    ...
    async def test_commit_changes(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert mock_run.called
            commit_calls = [call for call in mock_run.call_args_list if "commit" in str(call[0][0])]
