...
class TestMockClaudeClient:
    ...
    def test_mock_response_implementation(self):
        ...
        response = client.messages.create(
            model="test", messages=[{"role": "user", "content": "Should we implement this feature?"}], max_tokens=100
        )
    ...
    def test_mock_response_architecture(self):
        ...
        client = MockClaudeClient()
        response = client.messages.create(
            model="test", messages=[{"role": "user", "content": "What architecture should we use?"}], max_tokens=100
        )

        assert (
            "architectural" in response.content[0].text.lower()
            or "architecture" in response.content[0].text.lower()
            or "factors" in response.content[0].text.lower()
        )
    ...
    def test_mock_response_general(self):
        ...
        client = MockClaudeClient()
        response = client.messages.create(
            model="test", messages=[{"role": "user", "content": "What do you think?"}], max_tokens=100
        )
...
class TestMockOpenAIClient:
    ...
    def test_mock_response_complex(self):
        ...
        response = client.chat.completions.create(
            model="gpt-4", messages=[{"role": "user", "content": "Analyze this complex system"}], max_tokens=100
        )
    ...
    def test_mock_response_simple(self):
        ...
        client = MockOpenAIClient()
        response = client.chat.completions.create(
            model="gpt-4", messages=[{"role": "user", "content": "Simple question"}], max_tokens=100
        )

        assert (
            "gpt" in response.choices[0].message.content.lower()
            or "efficient" in response.choices[0].message.content.lower()
        )
...
class TestCachedClaudeClient:
    ...
    def test_cache_key_creation(self, temp_cache_dir):
        ...
        key = client._create_cache_key("claude-3", [{"role": "user", "content": "test message"}])
    ...
    def test_cache_miss_calls_api(self, temp_cache_dir):
        ...
        with patch("anthropic.Anthropic") as mock_anthropic:
            ...
            response = client.create(model="claude-3", messages=[{"role": "user", "content": "test"}])
    ...
    def test_cache_hit_avoids_api(self, temp_cache_dir):
        ...
        response = client.create(model="claude-3", messages=messages)
...
class TestCachedGeminiClient:
    ...
    async def test_cache_hit(self, temp_cache_dir):
        ...
        cache_data = {"response": "Cached Gemini response", "timestamp": "2024-01-01T00:00:00", "prompt": prompt}
    ...
    async def test_fallback_to_openai_on_error(self):
        ...
        with patch("google.generativeai.configure"):
            ...
            with patch("google.generativeai.GenerativeModel") as mock_model_class:
                ...
                mock_model.generate_content_async = AsyncMock(
                    side_effect=Exception("Error 429: Resource exhausted (quota)")
                )
                mock_model_class.return_value = mock_model
