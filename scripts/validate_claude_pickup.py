...
class ClaudeTaskValidator:
    def __init__(self):
        ...
        self.task_directories = [".claude-tasks", "ai-tasks", "TODO"]
    ...
    def _check_critical_files(self):
        ...
        for file in self.critical_files:
            ...
            if file_path.exists():
                ...
                if has_urgent:
                    self._add_result(f"File: {file}", "PASS", f"✅ File exists with urgent markers")
                else:
                    self._add_result(f"File: {file}", "WARN", f"⚠️  File exists but missing urgent markers")
            else:
                self._add_result(
                    f"File: {file}",
    ...
    def _check_task_directories(self):
        ...
        for dir_name in self.task_directories:
            ...
            if dir_path.exists() and dir_path.is_dir():
                ...
                urgent_tasks = [
                    f for f in task_files if "URGENT" in f.name.upper() or "178" in f.name or "180" in f.name
                ]

                if urgent_tasks:
                    self._add_result(
                        f"Directory: {dir_name}", "PASS", f"✅ Contains {len(urgent_tasks)} urgent task(s)"
                    )
                elif task_files:
                    self._add_result(
                    ...
                    self._add_result(f"Directory: {dir_name}", "INFO", f"ℹ️  Directory exists but empty")
            else:
                self._add_result(f"Directory: {dir_name}", "WARN", f"⚠️  Directory not found")
    ...
    def _check_readme_visibility(self):
        ...
        if readme_path.exists():
            ...
            if "URGENT" in top_lines and "Claude" in top_lines:
                self._add_result("README Visibility", "PASS", "✅ README has urgent Claude notice at top")
            else:
                self._add_result("README Visibility", "FAIL", "❌ README missing urgent notice in first 10 lines")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if pinned_issues:
                    ...
                    self._add_result("GitHub Issues", "WARN", f"⚠️  {len(open_issues)} open AI issue(s) but none pinned")
                else:
                    self._add_result("GitHub Issues", "INFO", "ℹ️  No open AI-assigned issues")
            else:
                self._add_result("GitHub Issues", "ERROR", f"❌ Failed to check issues: {result.stderr}")

        except Exception as e:
            self._add_result("GitHub Issues", "ERROR", f"❌ Error checking issues: {str(e)}")
    ...
    def _check_workflow_status(self):
        ...
        try:
            result = subprocess.run(["gh", "workflow", "list", "--json", "name,state"], capture_output=True, text=True)
            ...
            if result.returncode == 0:
                ...
                if active_ai:
                    self._add_result("AI Workflows", "PASS", f"✅ {len(active_ai)} active AI workflow(s)")
                else:
                    self._add_result("AI Workflows", "FAIL", "❌ No active AI workflows found")
            else:
                self._add_result("AI Workflows", "ERROR", f"❌ Failed to check workflows: {result.stderr}")

        except Exception as e:
            self._add_result("AI Workflows", "ERROR", f"❌ Error checking workflows: {str(e)}")

    def _add_result(self, check: str, status: str, message: str):
        """Add a validation result"""
        self.validation_results.append(
            {"check": check, "status": status, "message": message, "timestamp": datetime.now().isoformat()}
        )

        # Print immediately
        status_emoji = {"PASS": "✅", "FAIL": "❌", "WARN": "⚠️", "INFO": "ℹ️", "ERROR": "🚨"}
