...
class DecisionCriteria:
    ...
    def complexity_score(self) -> float:
        ...
        reversibility_score = {"irreversible": 1.0, "semi-reversible": 0.6, "reversible": 0.3}[self.reversibility]
        stakeholder_score = min(1.0, self.stakeholder_count / 10)  # Normalize to 0-1
        time_score = scores[self.time_sensitivity]
        
        # Weighted average
        return (impact_score * 0.3 + complexity_score * 0.3 + reversibility_score * 0.2 + 
                stakeholder_score * 0.1 + time_score * 0.1)
