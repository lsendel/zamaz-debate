...
from .events import DebateInitiated, RoundStarted, RoundCompleted, ConsensusReached, DecisionMade
...
class DebateSession:
    ...
    def __post_init__(self):
        ...
        if self.status == DebateStatus.INITIATED and not self._events:
            self._publish_event(DebateInitiated(
                debate_id=self.id,
    ...
    def start_round(self) -> Round:
        ...
        if self.rounds and not self.rounds[-1].is_completed:
            ...
            self._publish_event(RoundCompleted(
                debate_id=self.id,
        ...
        self._publish_event(RoundStarted(
            debate_id=self.id,
    ...
    def reach_consensus(self, consensus: Consensus) -> None:
        ...
        self._publish_event(ConsensusReached(
            debate_id=self.id,
    ...
    def make_decision(self, decision: 'Decision') -> None:
        ...
        self._publish_event(DecisionMade(
            debate_id=self.id,
...
class Decision:
    ...
    def requires_debate(self) -> bool:
        ...
        return self.type in [DecisionType.COMPLEX, DecisionType.ARCHITECTURAL, DecisionType.STRATEGIC]